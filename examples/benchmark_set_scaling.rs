/// Benchmark for measuring set operation throughput scaling across thread counts
///
/// This benchmark tests pure set workloads with random keys to measure:
/// - Throughput (operations per second) for each thread count
/// - How performance scales as threads increase
/// - Contention effects on the concurrent data structure
///
/// Run with: cargo run --release --example benchmark_set_scaling
...
/// Generate a pseudo-random key based on thread ID and counter
fn generate_key(thread_id: usize, counter: u64) -> [u8; 16] {
    // Use simple hash to distribute keys across key space
    let key_id = (thread_id as u64 * 1_000_000 + counter) % KEY_SPACE;
    let mut key = [0u8; 16];
    key[0..8].copy_from_slice(&key_id.to_le_bytes());
    key[8..16].copy_from_slice(&thread_id.to_le_bytes());
    key
}
    ...
    for thread_id in 0..num_threads {
        ...
        let handle = tokio::task::spawn(async move {
            ...
            while !done.load(Ordering::Relaxed) {
                let key = generate_key(thread_id, counter);
...
#[tokio::main]
async fn main() {
    println!("Segcache2 Set Operation Scaling Benchmark");
    println!("==========================================");
