...
pub enum HugepageSize {
    ...
    /// Size is rounded up to 2MB boundary (max ~2MB overhead).
    TwoMegabyte,
    /// 1GB hugepages (MAP_HUGETLB | MAP_HUGE_1GB).
    /// Falls back directly to regular pages (with THP hint) if unavailable.
    /// Does NOT fall back to 2MB - if you have 1GB pages configured,
    /// you probably don't have a 2MB pool.
    /// Size is rounded up to 1GB boundary if waste < 50%, otherwise to 2MB.
    OneGigabyte,
}
...
///
/// # Fallback Behavior
/// - `OneGigabyte`: Tries 1GB -> 2MB -> regular pages
/// - `TwoMegabyte`: Tries 2MB -> regular pages
/// - `None`: Uses regular pages directly
pub fn allocate(size: usize, hugepage_size: HugepageSize) -> Result<HugepageAllocation, std::io::Error> {
    if size == 0 {
